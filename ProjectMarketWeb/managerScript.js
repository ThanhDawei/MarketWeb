...
document.addEventListener("DOMContentLoaded", () => {
      ...
      if (item.quantity > 0) {
        ...
        html += `
                <tr>
                    ...
                    <td>
                        <button class="btn-view" onclick="alert('Xem chi tiết kho của ${escapeHtml(
                          item.productName
                        )}.')">Xem</button>
                    </td>
                </tr>
  ...
  // === HÀM LƯU SẢN PHẨM (THÊM/SỬA) VỚI RÀNG BUỘC KHO HÀNG (ĐÃ CẬP NHẬT) ===
  window.saveProduct = function (event) {
    ...
    if (isEditMode) {
      ...
      product.name = name;
      product.value = value;
      product.quantity = quantity; // Số lượng mới đã được kiểm tra ràng buộc
  ...
  // CẬP NHẬT: Ngừng tự động thêm/cập nhật sản phẩm vào products list
  window.submitImportReceipt = function (event) {
    ...
    // Bỏ qua việc cập nhật products list: Hàng chỉ ở trong kho (importReceipts)
...
━━━━━━━━━━━━━━━━━━━━━━
  ...
  window.viewInvoice = function (id) {
    ...
    const itemsStr = invoice.items
      .map((it) => `- ${it.name}: ${formatPrice(it.price)}đ`)
      .join("\n");
    alert(
      `HÓA ĐƠN #${id}\n\nNgày: ${invoice.date}\nKhách hàng: ${
        invoice.user
      }\n\nSản phẩm:\n${itemsStr}\n\nTổng: ${formatPrice(invoice.total)}đ`
    );
  };
